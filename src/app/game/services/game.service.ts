...
export class GameService {
    ...
    for (let row = 0; row < rowsCount; ++row) {
      cells.push(
        Array.from({ length: columnsCount }).map(
          () => new Cell(Math.random() < percentAlive)
        )
      );
    }
  ...
  getNextGeneration(cells: Array<Array<Cell>>) {
    ...
    let rowA = cells[0].map((_, index) =>
      this.calculateNextGenerationCell(0, index, cells)
    );
    let rowB = cells[1].map((_, index) =>
      this.calculateNextGenerationCell(1, index, cells)
    );

    for (let i = 2; i < cells.length; ++i) {
      nextGenerationCells[i - 2] = rowA;
      rowA = rowB;
      rowB = cells[i].map((_, index) =>
        this.calculateNextGenerationCell(i, index, cells)
      );
    }
  ...
  private calculateNextGenerationCell(
    row: number,
    column: number,
    ...
    const liveNeighborCount = this.getNeighborsOfCell(
      row,
      column,
      maxRowNumber,
      maxColumnNumber,
  ...
  private getNeighborsOfCell(
    ...
    const possibleNeighborCoords = [
      ...
      offset =>
        !this.isOutOfBounds(offset.row, offset.column, maxRow, maxColumn)
    );

    return possibleNeighborCoords.map(
      coords => cells[coords.row][coords.column]
    );
  }
